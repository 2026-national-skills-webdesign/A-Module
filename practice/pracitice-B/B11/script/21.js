...
function render() {
  ...
  if(run) {
    ...
    id = requestAnimationFrame(timer)
    container.innerHTML =  `<button onclick="render()">중지</button><button onclick="location.reload()">초기화</button>`
  } else {
    elapsed += performance.now() - startTime
    cancelAnimationFrame(id)
    container.innerHTML =  `<button onclick="render()">계속</button><button onclick="location.reload()">초기화</button>`
  }
}